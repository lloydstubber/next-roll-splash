...
function isCompletedNavigationTiming(win, entry) {
    var _a;
    return isNavigationTiming(entry) && ((_a = win.document) === null || _a === void 0 ? void 0 : _a.readyState) === 'complete' && entry.loadEventEnd > 0;
}
// an `entryTypes` observation never delivers an entry that completed before it started
function completedNavigationEntries(win) {
    var _a;
    if (((_a = win.document) === null || _a === void 0 ? void 0 : _a.readyState) !== 'complete') {
        return [];
    }
    return win.performance
        .getEntriesByType('navigation')
        .filter(function (entry) { return isCompletedNavigationTiming(win, entry); });
}
...
function initialEntries(win, options) {
    if (options.recordInitialRequests) {
        return win.performance
            .getEntries()
            .filter(function (entry) {
            return isCompletedNavigationTiming(win, entry) ||
...
function initPerformanceObserver(cb, win, options) {
    ...
    try {
        cb({
            requests: initialEntries(win, options).flatMap(function (entry) {
...
function _tryReadBodyStreaming(r, limitBytes) {
    ...
    return new Promise(function (resolve) {
        ...
        function cancel() {
            try {
                void (reader === null || reader === void 0 ? void 0 : reader.cancel().catch(function () { }));
            }
            catch (_a) {
    ...
    var restorePatch = (0, patch_1.patch)(win, 'fetch', function (originalFetch) {
        return function (url, init) {
            return __awaiter(this, void 0, void 0, function () {
                var req, res, networkRequest, start, end, requestHeaders_2, requestBodyIsReadableStream, _a, e_3, _b, responseHeaders_1, _c, e_4;
                return __generator(this, function (_d) {
                    switch (_d.label) {
                        case 0:
                            try {
                        ...
                        case 14:
                            e_4 = _d.sent();
                            logger.error('Failed to record fetch response for network capture', e_4);
                            return [3 /*break*/, 15];
