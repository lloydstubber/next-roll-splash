...
function renderGreetingLine(text, styles) {
    var links = /(!|\\)?\[([^\]\n]+)\]\(([^()\s]+)\)/g;
    var parts = [];
    var lastIndex = 0;
    var match;
    while ((match = links.exec(text))) {
        ...
        var _a = __read(match, 4), prefix = _a[1], label = _a[2], url = _a[3];
        var safeUrl = !prefix && label.indexOf('[') === -1 && sanitizeUrl(url);
        var literal = prefix === '\\' ? match[0].slice(1) : match[0];
        parts.push(safeUrl
            ? renderTextWithMarks(label, [{ type: 'link', attrs: { href: safeUrl } }], styles, "".concat(match.index))
            : literal);
 ...
 */
function renderPlainText(text, styles, isGreeting) {
    if (!text) {
        return (0, jsx_runtime_1.jsx)(jsx_runtime_1.Fragment, {});
    }
    var lines = text.split('\n');
    return ((0, jsx_runtime_1.jsx)(jsx_runtime_1.Fragment, { children: lines.map(function (line, index) { return ((0, jsx_runtime_1.jsxs)(preact_1.Fragment, { children: [isGreeting ? renderGreetingLine(line, styles) : line, index < lines.length - 1 && (0, jsx_runtime_1.jsx)("br", {})] }, index)); }) }));
}
/**
...
function RichContent(_a) {
    var richContent = _a.richContent, content = _a.content, isCustomer = _a.isCustomer, primaryColor = _a.primaryColor, isGreeting = _a.isGreeting;
